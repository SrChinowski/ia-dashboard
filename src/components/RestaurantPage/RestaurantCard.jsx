...
const RestaurantCard = ({data}) => {
    ...
    const [anchorRestaurantMenu, setAnchorRestaurantMenu] = useState(null);

    //Menu Handlers
    const openRestaurantMenu = (event) => {
        setAnchorRestaurantMenu(event.currentTarget);
        setOpenMenu(true)
    }

    const closeRestaurantMenu = () => {
        setAnchorRestaurantMenu(null);
        setOpenMenu(false)
    }
            ...
            <div
                ...
                <span className="grid flex flex-col space-y-2">
                    <span className="transition duration-75 ease-in-out justify-self-end p-1 m-1 mr-0 hover:bg-gray-100 rounded-full">
                        ...
                        onMouseEnter={openRestaurantMenu} /> 

                        <Popper open={openMenu} anchorEl={anchorRestaurantMenu} role={undefined} transition disablePortal>
                    ...
                    <p className="text-gray-500" style={{marginTop: "0"}}>{data.subtitle}</p>
                    <div className={clsx(
                        classes.state, {
                        [classes.stateOpen]: data.open,
                    })}>
